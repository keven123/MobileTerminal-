...
static NSString* kKeyboardSettings = @"keyboardSettings";
...
- (void)persist
{
  ...
  NSData* color = [NSKeyedArchiver archivedDataWithRootObject:self];
  [[NSUserDefaults standardUserDefaults] setObject:data forKey:kSettingsKey];
  [[NSUserDefaults standardUserDefaults] setObject:color forKey:kKeyboardSettings];
}
...
- (id)initWithCoder:(NSCoder *)decoder
{
  ...
  if (self != nil) {
    ...
    if ([decoder containsValueForKey:kTerminalSettings]) {
      ...
      if ([decoder containsValueForKey:kKeyboardSettings]) {
          keyboardSettings = [[decoder decodeObjectForKey:kKeyboardSettings] retain];
      } else {
          keyboardSettings = [[KeyboardSettingsViewController alloc] init];
      }
  }
  return self;
}
...
- (void)encodeWithCoder:(NSCoder *)encoder
{
  ...
  [encoder encodeObject:keyboardSettings forKey:kKeyboardSettings];
